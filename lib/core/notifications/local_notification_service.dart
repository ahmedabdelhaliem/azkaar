...
class LocalNotificationService {
  ...
  static Future<void> showMultipleScheduledNotifications() async {
    const AndroidNotificationDetails android = AndroidNotificationDetails(
    ...
    // تأكيد تهيئة منطقة الوقت
    tz.initializeTimeZones();
    tz.setLocalLocation(tz.getLocation('Africa/Cairo'));

    var currentTime = tz.TZDateTime.now(tz.local);
    var startTime = tz.TZDateTime(
        tz.local, currentTime.year, currentTime.month, currentTime.day, 14, 47);

    if (currentTime.isAfter(startTime)) {
      startTime = startTime.add(const Duration(days: 1));
    }
    List<Map<String, String>> adkarJson = [
      {
    ...
    for (int i = 0; i < adkarJson.length; i++) {
      var scheduledTime = startTime.add(Duration(minutes: i * 5));

      await FlutterLocalNotificationsPlugin().zonedSchedule(
        i, // معرّف الإعلام
        adkarJson[i]['category'], // العنوان
        adkarJson[i]['content'], // المحتوى
        scheduledTime, // الوقت المجدول
        details, // تفاصيل الإعلام
        // السماح بالإعلام أثناء وضع السكون
        uiLocalNotificationDateInterpretation:
            UILocalNotificationDateInterpretation.wallClockTime,
        matchDateTimeComponents: DateTimeComponents.time, // تطابق وقت الإعلام
